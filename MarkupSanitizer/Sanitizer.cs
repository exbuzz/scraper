...
namespace MarkupSanitizer
{
    public static class Sanitizer
    {
        ...
        static IDictionary<string, TagDefinition> _allowedTags;
        ...
        public static SanitizedMarkup SanitizeMarkup(string inputMarkup)
        {
            ...
            var markupText = writer.ToString();

            for (var i = 0; i < EmptyTagPassCount; i++ )
            {
                markupText = EmptyTagRegex.Replace(
                    markupText,
                    match =>
                        {
        ...
        internal static IMarkupToken[] TokenizeMarkup(string markup)
        {
        ...
        {
            var unescapedText = CharacterEscapeRegex.Replace(rawText, match => System.Web.HttpUtility.HtmlDecode(match.Value));
        ...
        static IDictionary<string, TagDefinition> GetAllowedTags()
        {
            ...
            {
                var standardContentHolders = new[] { "p", "li", "td", "dt", "dd" };
                ...
                foreach (var tagDefinition in allowedTagsList)
                {
                    ...
                    if (tagDefinition.LegacyNames == null) continue;
                    foreach (var legacyTag in tagDefinition.LegacyNames)
                    {
